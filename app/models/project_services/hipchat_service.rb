class HipchatService < Service
  ...
  def fields
    [
      { type: 'text', name: 'token',     placeholder: '房间令牌' },
      { type: 'text', name: 'room',      placeholder: '房间名称或ID' },
      { type: 'checkbox', name: 'notify' },
      { type: 'select', name: 'color', choices: ['yellow', 'red', 'green', 'purple', 'gray', 'random'] },
      { type: 'text', name: 'api_version',
        placeholder: '默认为空白（v2）' },
      { type: 'text', name: 'server',
        placeholder: '默认为空。 https://hipchat.example.com' },
  ...
  def create_push_message(push)
    ...
    if Gitlab::Git.blank_ref?(before)
      ...
      message << "推送了 #{ref_type} <a href=\""\
                  "#{project.web_url}/commits/#{CGI.escape(ref)}\">#{ref}</a>"
      message << "的 <a href=\"#{project.web_url}\">#{project.name_with_namespace.gsub!(/\s/, '')}</a> "
