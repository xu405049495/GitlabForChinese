module BlobHelper
  ...
  def edit_blob_link(project = @project, ref = @ref, path = @path, options = {})
    ...
    if !on_top_of_branch?(project, ref)
      button_tag "编辑", class: "btn disabled has-tooltip btn-file-option", title: "您只能在分支上修改编辑文件", data: { container: 'body' }
  ...
  def modify_file_link(project = @project, ref = @ref, path = @path, label:, action:, btn_class:, modal_type:)
    ...
    if !on_top_of_branch?(project, ref)
      button_tag label, class: "btn btn-#{btn_class} disabled has-tooltip", title: "您只能在分支上#{action}文件", data: { container: 'body' }
  ...
  def licenses_for_select
    ...
    @licenses_for_select = {
        常用: licenses.select(&:featured).map { |license| { name: license.name, id: license.key } },
        其他: licenses.reject(&:featured).map { |license| { name: license.name, id: license.key } }
    }
  end
